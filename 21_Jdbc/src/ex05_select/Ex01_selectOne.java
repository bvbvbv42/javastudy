...
public class Ex01_selectOne {
  ...
  public static void main(String[] args) {
    ...
    // ResultSet 객체 선언 (select 검색 결과 처리)
    ResultSet rs = null;
    
    try {
      
      // Connection 객체 생성 (DB_Connect 클래스의 getConnection 메소드로부터 받아오기)(만든거 가져옴)
      ...
      // 쿼리문 실행         (insert 와 다른점 첫번째 : 쿼리문 실행)
      rs = ps.executeQuery();
      ...
      if(rs.next()) {               // 만약 조회결과가 있다면,
      
        // 검색 결과 행 → UserDto 객체 생성
        user = new UserDto();
        user.setUser_no(rs.getInt("USER_NO"));
        user.setUser_id(rs.getString("USER_NO"));
