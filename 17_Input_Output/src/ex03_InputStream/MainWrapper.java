...
public class MainWrapper {
  ...
  public static void ex02() {
    ...
    try {
      ...
      // 실제로 읽은 바이트 수 저장 변수
      int readByte = 0;               // 2번 텍스크 크기 -> 15byte = 4 4 4 3
      
      // 입력된 데이터를 누적할 StringBuilder 생성
      StringBuilder sb = new StringBuilder();
      
      // read(byte[] b) 메소드
      // 1. 파라미로 전달된 byte[] b에 읽은 내용을 저장한다.
      // 2. 실제로 읽은 바이트 수를 반환한다.
      // 2. 읽은 내용이 없으면 -1을 반환한다.
      
      /*
      readByte = fin.read(b);                                 // 4가 넘어감
      sb.append( new String(b, 0, readByte) );                // b리드배열에 리드바이트만큼
      */
