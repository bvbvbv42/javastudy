...
public class MainWrapper {
  ...
  public static void ex02() {
    ...
    // 파일을 File 객체로 생성
    File file = new File(dir, "myfile.txt");       // 폴더가 부모 파일이 자식
    ...
    if(file.exists()) {
      ...
      file.createNewFile();                        // 제일 쓸데 없는 코드, 반드시 예외 처리를 해야 하는 코드 (Checked Exception인 IOException 발생)
      System.out.println("myfile.txt 파일 생성 완료");
    }
  
    } catch(IOException e) {
      e.printStackTrace();        // 예외가 어디서 발생했는지 알려줘
    }
  ...
  public static void ex03() {
    ...
    // (제일중요) *********디렉터리에 있는 모든 File 객체(파일, 디렉터리) 가져오기***********
